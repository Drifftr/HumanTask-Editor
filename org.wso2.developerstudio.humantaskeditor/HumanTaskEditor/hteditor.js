 ...
 */
///////////////////////////////////////////////////////
/* input mapping sync to wsdl
 * alert through a dialog box - (Console log) Jira Tooling 
 * makedirty when something edited -  only dirty thing is synced - Jira tooling request
...
function generateInputWSDL(wsdlDom, currentTask) {
	...
	inputElements.each(function(i, obj) {
		if (i != 0) { // skip header
			...
			type = "xsd:"
					+ $("#" + currentTaskName + " #taskInputMappingType" + k)
							.val();
	...
	newPart.setAttribute("element", "tschema:"
			+ newElement.getAttribute("name")); // Data binding
	...
	newWSDLBindingSoapOperation.setAttribute("soapAction", "urn:"
			+ operationName);
...
function generateOutputWSDL(wsdlDom, currentTask) {
	...
	outputElements.each(function(i, obj) {
		if (i != 0) {
			...
			type = "xsd:"
					+ $("#" + currentTaskName + " #taskOutputMappingType" + k)
							.val();
			newSubElement = wsdlDom.createElementNS(
					"http://www.w3.org/2001/XMLSchema", "xsd:element");
			newSubElement.setAttribute("name", $(this).find(
	...
	newPart.setAttribute("element", "tschema:"
			+ newElement.getAttribute("name")); // Data
	// binding
	newMessage.appendChild(newPart);
	wsdlDefinitions.appendChild(newMessage);
	// create portType
...
function execUndoableOperation() {
	/*
	 * undoOpCount++; var label = "Operation " + undoOpCount; var uniqueID =
	 * "OperationID" + undoOpCount;
	 * 
...
function generateUI() { 
	...
	nodes.forEach(function(taskNode) {
		...
		$("#tasksList").append(
				"<li><a id='" + taskName + "Link' href='" + taskName
						+ "'><label>" + taskName + "</label></a></li>");
...
function generateTaskDiv(taskNode) {
	...
	$('#' + taskDivName + " #addInput")
			...
			.click(
					function(e) {
						...
						inputNode = taskNode.getElementsByTagName("renderings")[0]
								.getElementsByTagName("inputs")[0];
						newElement = xmlDom.createElementNS(
								"http://wso2.org/ht/schema/renderings/",
								"wso2:element");
						newElement.setAttribute("id", "Dummy"); //place holder
						...
						mapping = '<tr id="inputmapping'
								+ mappingNo
								+ '"><td width="25%"><input name="taskInputMappingid" type="hidden" id="taskInputMappingid" value="'
								+ mappingNo
								+ '"><input name="textfield6" type="text" id="taskInputMappingElementName'
								+ mappingNo
								+ '" value="dummy"></td><td width="25%"><input name="textfield7" type="text" id="taskInputMappingDisplayName'
								+ mappingNo
								+ '" value="dummy"></td><td width="10%"><input name="textfield8" type="text" id="taskInputMappingOrder'
								+ mappingNo
								+ '" value="dummy"></td><td width="20%"><label><select id="taskInputMappingType'
								+ mappingNo
								+ '" name="select3"><option value="string" selected>string</option><option value="int">int</option><option value="double">double</option><option value="float">float</option><option value="boolean">boolean</option><option value="organizationalEntity">organizationalEntity</option></select></label></td><td><label><input type="button" class="inputDeleteButton" name="deleteButton'
								+ mappingNo + '" id="deleteButton' + mappingNo
								+ '" value="Delete"></label></td></tr>';
						$(
								"#" + taskNode.getAttribute("name")
										+ "wrapper #inputmappingTable").append(
								mapping);

						// delete button click event
						$(
								'#' + taskNode.getAttribute("name")
										+ 'wrapper .inputDeleteButton')
								.off()
								.click(
										function(er) {
											er.preventDefault();
			...
			.click(
					function(el) {
						try {
							...
							newElement.appendChild(newXpath);
							newValue = xmlDom.createElementNS(
									"http://wso2.org/ht/schema/renderings/",
									"wso2:value");
							...
							newElement.appendChild(newDefault);
							outputNode.appendChild(newElement);

							// bind click event
							el.preventDefault();
							var outputmappingNo = parseInt($(
									'#' + taskDivName + " #taskOutputMappingNo")
									.val());
							outputmapping = '<tr id="outputmapping'
									+ outputmappingNo
									+ '"><td width="25%"><input name="taskOutputMappingid" type="hidden" id="taskOutputMappingid" value="'
									+ outputmappingNo
									+ '"><input name="textfield6" type="text" id="taskOutputMappingElementName'
									+ outputmappingNo
									+ '" value="dummy"></td><td width="25%"><input name="textfield7" type="text" id="taskOutputMappingDisplayName'
									+ outputmappingNo
									+ '" value="dummy"></td><td width="10%"><input name="textfield8" type="text" id="taskOutputMappingOrder'
									+ outputmappingNo
									+ '" value="dummy"></td><td width="20%"><label><select id="taskOutputMappingType'
									+ outputmappingNo
									+ '" name="select3"><option value="string" selected>string</option><option value="int">int</option><option value="double">double</option><option value="float">float</option><option value="boolean">boolean</option><option value="organizationalEntity">organizationalEntity</option></select></label></td><td width="10%"><label><input name="textfield12" type="text" id="taskOutputMappingDefaultValues'
									+ outputmappingNo
									+ '" value="dummy"></label></td><td><label><input type="button" class="outputDeleteButton" name="outputDeleteButton'
									+ outputmappingNo
									+ '" id="outputDeleteButton'
									+ outputmappingNo
									+ '" value="Delete"></label></td></tr>';
							$(
									"#" + taskNode.getAttribute("name")
											+ "wrapper #outputmappingTable")
									.append(outputmapping);
							// delete button click event
							$(
									'#' + taskNode.getAttribute("name")
											+ 'wrapper .outputDeleteButton')
									.off()
									.click(
											function(eo) {
												eo.preventDefault();
		...
		if (inputNodes[i].nodeName != "#text") {
			try {
				...
				mapping = '<tr id="inputmapping'
						...
						+ mappingNo
						+ '" value="'
						+ inputNodes[i].getAttribute("id")
						+ '"/></td><td width="25%"><input name="textfield7" type="text" id="taskInputMappingDisplayName'
						+ mappingNo
						+ '" value="'
						+ inputNodes[i].getElementsByTagName("label")[0].childNodes[0].nodeValue
						+ '"/></td><td width="10%"><input name="textfield8" type="text" id="taskInputMappingOrder'
						+ mappingNo
						+ '" value="'
						+ inputNodes[i].getElementsByTagName("value")[0].childNodes[0].nodeValue
						+ '"/></td><td width="20%"><label><select id="taskInputMappingType'
						+ mappingNo
						+ '" name="select3"><option value="string" selected>string</option><option value="int">int</option><option value="double">double</option><option value="float">float</option><option value="boolean">boolean</option><option value="organizationalEntity">organizationalEntity</option></select></label></td><td><label><input type="button" class="inputDeleteButton" name="deleteButton'
						+ mappingNo + '" id="deleteButton' + mappingNo
						+ '" value="Delete"></label></td></tr>';
		...
		if (outputNodes[i].nodeName != "#text") {
			try {
				...
				outputmapping = '<tr id="outputmapping'
						...
						+ outputmappingNo
						+ '" value="'
						+ outputNodes[i].getAttribute("id")
						+ '"></td><td width="25%"><input name="textfield7" type="text" id="taskOutputMappingDisplayName'
						+ outputmappingNo
						+ '" value="'
						+ outputNodes[i].getElementsByTagName("label")[0].childNodes[0].nodeValue
						+ '"></td><td width="10%"><input name="textfield8" type="text" id="taskOutputMappingOrder'
						+ outputmappingNo
						+ '" value="'
						+ outputNodes[i].getElementsByTagName("value")[0].childNodes[0].nodeValue
						+ '"></td><td width="20%"><label><select id="taskOutputMappingType'
						+ outputmappingNo
						+ '" name="select3"><option value="string" selected>string</option><option value="int">int</option><option value="double">double</option><option value="float">float</option><option value="boolean">boolean</option><option value="organizationalEntity">organizationalEntity</option></select></label></td><td width="10%"><label><input name="textfield12" type="text" id="taskOutputMappingDefaultValues'
						+ outputmappingNo
						+ '" value="'
						+ outputNodes[i].getElementsByTagName("default")[0].childNodes[0].nodeValue
						+ '"></label></td><td><label><input type="button" class="outputDeleteButton" name="outputDeleteButton'
						+ outputmappingNo + '" id="outputDeleteButton'
						+ outputmappingNo
						+ '" value="Delete"></label></td></tr>';
	...
	$(
			'#' + taskDivName
					+ " input[name=potentialOwners][value=potentialOwners"
					+ ownerType + "]").prop("checked", true);
	...
	$(
			'#'
					+ taskDivName
					+ " input[name=businessAdministrators][value=businessAdministrators"
					+ ownerType + "]").prop("checked", true);
...
function generateText(taskNode) {
	...
	for (i = 0; i < inputNodes.length; i++) {
		if (inputNodes[i].nodeName != '#text') {
			inputNodes[i].setAttribute("id", $(
					'#' + taskDivName + " #taskInputMappingElementName"
							+ inputmapping).val());
			inputNodes[i].getElementsByTagName("label")[0].childNodes[0].nodeValue = $(
					'#' + taskDivName + " #taskInputMappingDisplayName"
							+ inputmapping).val();
			inputNodes[i].getElementsByTagName("value")[0].childNodes[0].nodeValue = $(
					'#' + taskDivName + " #taskInputMappingOrder"
							+ inputmapping).val();
			inputmapping++;
		}
	}
	...
	for (i = 0; i < outputNodes.length; i++) {
		if (outputNodes[i].nodeName != '#text') {
			outputNodes[i].setAttribute("id", $(
					'#' + taskDivName + " #taskOutputMappingElementName"
							+ outputmapping).val());
			outputNodes[i].getElementsByTagName("label")[0].childNodes[0].nodeValue = $(
					'#' + taskDivName + " #taskOutputMappingDisplayName"
							+ outputmapping).val();
			outputNodes[i].getElementsByTagName("value")[0].childNodes[0].nodeValue = $(
					'#' + taskDivName + " #taskOutputMappingOrder"
							+ outputmapping).val();
			outputNodes[i].getElementsByTagName("default")[0].childNodes[0].nodeValue = $(
					'#' + taskDivName + " #taskOutputMappingDefaultValues"
							+ outputmapping).val();
			outputmapping++;
		}
	}
		...
		taskNode.getElementsByTagName("peopleAssignments")[0]
				...
				.setAttribute(
						...
						getArgumentName($(
								'#'
										+ taskDivName
										+ " input[name=potentialOwners]:checked")
								.val()));
		...
		taskNode.getElementsByTagName("peopleAssignments")[0]
				...
				.setAttribute(
						...
						getArgumentName($(
								'#'
										+ taskDivName
										+ " input[name=businessAdministrators]:checked")
								.val()));
	...
	if (taskNode.getElementsByTagName("peopleAssignments")[0]
		...
		taskNode.getElementsByTagName("peopleAssignments")[0]
				...
				getArgumentName($(
						'#' + taskDivName
								+ " input[name=taskStakeholders]:checked")
						.val()));
